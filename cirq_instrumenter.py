...
def generate_header():
    ...
    # Enhanced with Cirq-specific structural state serializers.
    return """# -*- coding: utf-8 -*-
# vim: expandtab tabstop=4 shiftwidth=4
# -*- indent-tabs-mode: nil; tab-width: 4 -*-
# ==========================================
# CIRQ-OPTIMIZED RECURSIVE WRAPPER (PURE SPACES)
# ==========================================
...
def _serialize_cirq_obj(obj):
    \"\"\"Extracts meaningful state from complex quantum objects for tabular reporting.\"\"\"
    try:
        obj_type = type(obj).__name__
        obj_mod = type(obj).__module__
        
        if 'cirq' in obj_mod or 'cirq' in obj_type.lower():
            # Handle Circuits
            if obj_type == 'Circuit':
                return f"Circuit(Moments: {len(obj)}, Qubits: {len(obj.all_qubits())})"
            # Handle Operations / Gates
            if hasattr(obj, 'gate') and hasattr(obj, 'qubits'):
                return f"Op: {obj.gate} on {obj.qubits}"
            # Handle Qubits
            if obj_type in ('GridQubit', 'LineQubit', 'NamedQubit'):
                return repr(obj)
            # Handle Simulation Results
            if 'Result' in obj_type or 'SimulationTrialResult' in obj_type:
                if hasattr(obj, 'measurements'):
                    measurements_summary = {k: v.shape for k, v in obj.measurements.items()}
                    return f"SimulationResult(Measurements: {measurements_summary})"
                return f"Result: {obj_type}"
                
        # Handle state vectors / density matrices
        if 'numpy' in obj_mod and hasattr(obj, 'shape'):
            if obj.ndim == 1 and obj.shape <= 16:  # Small state vector
                return f"StateVec: {list(obj.round(3))}"
...
def _reset_logs():
    ...
    try:
        with open("_VARIABLE_TRACKER.csv", "w", encoding="utf-8", newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["Line", "Variable", "Type", "Value"])
    except:
        pass
    ...
    try:
        with open("_VARIABLE_TRACKER.csv", "a", encoding="utf-8", newline='') as f:
            ...
            for var_name, var_val in local_vars.items():
                if var_name.startswith('_') or var_name in ('builtins', 'sys', 'os', 'csv'): 
                    continue
                v_type = type(var_val).__name__
                clean_val = _serialize_cirq_obj(var_val).replace('\\n', ' ').replace('\\r', '')
                writer.writerow([line_no, var_name, v_type, clean_val])
    except:
        pass
...
def inject_into_file(file_path, max_depth=3):
    try:
        ...
        for idx in range(total_lines):
            ...
            is_structural = (
                any(stripped_for_check.startswith(k) for k in structural_keywords)
